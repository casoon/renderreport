...
fn main() -> renderreport::Result<()> {
    ...
    let report = engine
        ...
        .add_component(
            Label::new("This is a Label — colored and sized")
                .with_size("14pt")
                .with_color("#2563eb"),
        )
        ...
        .add_component(
            Grid::new(3)
                .add_item(NumberField::new(1234567.89).with_prefix("$").as_item())
                .add_item(
                    NumberField::new(0.947)
                        .with_suffix("%")
                        .with_format("0.0")
                        .as_item(),
                )
                .add_item(NumberField::new(42195.0).with_suffix(" users").as_item()),
        )
        // ── Metric & Score Cards ────────────────────────────────
        ...
        .add_component(
            Grid::new(4)
                .add_item(
                    ScoreCard::new("Quality", 94)
                        .with_description("A+ rating")
                        .as_item(),
                )
                .add_item(
                    ScoreCard::new("Security", 87)
                        .with_description("Strong")
                        .as_item(),
                )
                .add_item(
                    ScoreCard::new("Speed", 72)
                        .with_description("Needs work")
                        .with_thresholds(80, 50)
                        .as_item(),
                )
                .add_item(
                    ScoreCard::new("Coverage", 45)
                        .with_description("Poor")
                        .with_thresholds(80, 50)
                        .as_item(),
                ),
        )
        ...
        .add_component(Chart::pie("Pie Chart").add_series(
            "Data",
            vec![
                ("Segment A".into(), 40.0),
                ("Segment B".into(), 30.0),
        ...
        .add_component(
            Grid::new(3)
                .add_item(Gauge::new("CPU", 65.0).with_range(0.0, 100.0).as_item())
                .add_item(Gauge::new("Memory", 82.0).with_range(0.0, 100.0).as_item())
                .add_item(
                    Gauge::thermometer("Temp", 38.5)
                        .with_range(20.0, 60.0)
                        .as_item(),
                ),
        )
        .add_component(
            Sparkline::bar(vec![3.0, 5.0, 4.0, 7.0, 6.0, 9.0, 8.0, 11.0]).with_color("#3b82f6"),
        )
        .add_component(
            Sparkline::bar(vec![12.0, 10.0, 14.0, 11.0, 16.0, 13.0, 18.0, 15.0])
                .with_color("#22c55e"),
        )
        // ── Findings ────────────────────────────────────────────
        ...
        .add_component(
            Finding::new(
                "Critical Issue",
                Severity::Critical,
                "System-wide outage detected. Immediate action required.",
            )
            .with_category("Infra"),
        )
        .add_component(
            Finding::new(
                "High Severity",
                Severity::High,
                "Memory leak in worker process causes gradual degradation.",
            )
            .with_recommendation("Restart workers on 4-hour schedule until patch is deployed.")
        ...
        .add_component(
            Finding::new(
                "Medium Severity",
                Severity::Medium,
                "Deprecated API endpoint still receiving 500 requests/day.",
            )
            .with_category("API"),
        )
        .add_component(
            Finding::new(
                "Low Severity",
                Severity::Low,
                "Minor UI alignment issue on mobile Safari.",
            )
        ...
        .add_component(
            Finding::new(
                "Informational",
                Severity::Info,
                "New monitoring dashboard deployed successfully.",
            )
        ...
        .add_component(
            Callout::success("Success callout for positive outcomes.").with_title("Success"),
        )
        .add_component(
            Callout::warning("Warning callout for potential issues.").with_title("Warning"),
        )
